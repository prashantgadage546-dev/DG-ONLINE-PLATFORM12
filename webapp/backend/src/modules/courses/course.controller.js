...
exports.getCourse = async (req, res) => {
  try {
    ...
    // Get lectures for this course (optionally include progress for logged-in user)
    const hasUser = Boolean(req.user?.id);
    const progressJoin = hasUser
      ? `
        LEFT JOIN lecture_progress lp
          ON lp.lecture_id = l.id
         AND lp.user_id = ${Number(req.user.id)}
      `
      : '';

    const progressSelect = hasUser
      ? `
    ...
    const topicsWithLectures = topics.map((topic) => ({
      ...topic,
      lectures: lectures.filter(
        (lecture) => Number(lecture.topic_id) === Number(topic.id)
      ),
    }));

    const course = {
      ...courses[0],
      topics: topicsWithLectures,
      lectures
    };
